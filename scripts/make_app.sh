...
#
# This script intentionally stops at an unsigned bundle. build_release.sh
# injects the release version, signs with a Developer ID certificate, notarizes
# and staples, then creates the distributable ZIP and Homebrew Cask. Never
# distribute build/AgentNotch.app directly.
set -euo pipefail

cd "$(dirname "$0")/.."
config="${1:-release}"
app="build/AgentNotch.app"
version="${AGENT_NOTCH_VERSION:-}"
build_number="${AGENT_NOTCH_BUILD_NUMBER:-}"
...
if [ -n "$version" ] || [ -n "$build_number" ]; then
  if [[ ! "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "AGENT_NOTCH_VERSION must use the form 1.2.3" >&2
    exit 1
  fi
