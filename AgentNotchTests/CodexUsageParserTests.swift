...
struct CodexUsageParserTests {
    ...
    @Test("Parses an individual allowance when newer rollouts provide it")
    func parsesIndividualLimit() throws {
        let line = """
            {"type":"event_msg","payload":{"type":"token_count","rate_limits":{"primary":null,"secondary":null,"individual_limit":{"limit":"60000","used":"15000","remaining_percent":75,"resets_at":1785542400},"plan_type":"business"}}}
            """

        let snapshot = try #require(CodexUsageParser.parseLine(line))
        let allowance = try #require(snapshot.individualLimit)
        #expect(allowance.used == 15_000)
        #expect(allowance.limit == 60_000)
        #expect(allowance.remainingPercent == 75)
        #expect(allowance.usedPercent == 25)
    }
    ...
    // MARK: - latestRolloutFile (YYYY/MM/DD lookup)
    ...
    // MARK: - latestSnapshot (tail-read fallback behavior)
    ...
    func latestSnapshotFallsBackToNilWhenTokenCountOutsideTailChunk() throws {
        ...
        // Put token_count first, then pile on padding lines to push it outside the tail chunk.
        let content =
            tokenCountLine + "\n"
    ...
    func latestSnapshotFindsTokenCountWithinTailChunk() throws {
        ...
        let tokenCountLine = """
            ...
            """
        let content =
            Array(repeating: paddingLine, count: 5).joined(separator: "\n")
            + "\n" + tokenCountLine + "\n"
        let path = dayDir + "/rollout-2026-07-25T00-00-00-zzz.jsonl"
        try content.write(toFile: path, atomically: true, encoding: .utf8)
