...
struct ToolSummaryTests {
    ...
    @Test(
        "Each tool family has a stable user-facing fallback",
        arguments: [
            ("Bash", [:], "bash"),
            ("Read", ["path": "/tmp/README.md"], "README.md"),
            ("Write", [:], "Write"),
            ("Grep", [:], "grep"),
            ("Glob", ["pattern": "**/*.swift"], "**/*.swift"),
            ("Glob", [:], "glob"),
            ("WebSearch", ["query": "Swift concurrency"], "Swift concurrency"),
            ("WebSearch", [:], "search"),
            ("WebFetch", ["url": "https://example.com/docs"], "example.com"),
            ("WebFetch", ["url": "not a URL"], "fetch"),
            ("Agent", ["subagent_type": "explorer"], "explorer"),
            ("Agent", [:], "agent"),
            ("mcp__incomplete", [:], "mcp__incomplete"),
        ] as [(String, [String: String], String)]
    )
    func toolFamilyFallbacks(
        toolName: String,
        input: [String: String],
        expected: String
    ) {
        #expect(
            ToolSummary.generate(toolName: toolName, toolInput: input)
                == expected
        )
    }
}
