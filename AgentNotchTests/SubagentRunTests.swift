...
struct SubagentRunTests {
    ...
    func stopFallsBackToAgentTypeFIFO() {
        ...
        // A Stop without an agentId matches the oldest running run of the same agentType.
        ...
        // The oldest one (t0) should be the completed one.
    ...
    func stopFallsBackToOldestRunningOverall() {
        ...
        // Falls back to the oldest running run even when the agentType does not match.
        let matched = session.stopSubagent(
            agentId: "unknown-id", agentType: "unknown-type", transcriptPath: nil)
    ...
    func doubleStopIsIgnored() {
        ...
        // A second Stop finds no running run to match, so it returns false.
    ...
    func trimsCompletedBeyond50() {
        ...
        // The newest (agent-59) survives; the oldest (agent-0) has been evicted.
