...
struct AgentTaskReconcilerTests {
    ...
    func toolThenHookPromotesId() {
        ...
        tasks = AgentTaskReconciler.reconcileCreated(tasks: tasks, info: hookInfo, now: now.addingTimeInterval(2))
    ...
    func hookThenToolSkipsDuplicate() {
        ...
        tasks = AgentTaskReconciler.reconcileCreated(tasks: tasks, info: toolInfo, now: now.addingTimeInterval(2))

        // 重複として skip され、既存の 1 件のまま
        #expect(tasks.count == 1)
    ...
    func outsideDedupWindowAppendsNew() {
        ...
        // 20 秒後の同名タスクは重複とみなさず追加される
        let second = TaskCreatedInfo(
            sessionId: "s1", subject: "Fix auth", description: "desc again",
            taskId: nil, assignee: nil, teamName: nil, source: .tool
        )
        tasks = AgentTaskReconciler.reconcileCreated(tasks: tasks, info: second, now: now.addingTimeInterval(20))
    ...
    func completedMatchesByTaskId() {
        ...
        tasks = AgentTaskReconciler.reconcileCompleted(tasks: tasks, info: completedInfo, now: now.addingTimeInterval(5))
